function [soln,err,integrator,ETE_integrator,Primal,Error] = ETEsolver(soln,err,integrator,ETE_integrator,bndry_cond,max_steps,out_interval)
    err = err.reset;
    p = [1,2,inf];
    N = length(p);
%     out_interval = max_steps;
    ...
    while (err.count < max_steps)&&(err.t(err.ptr(M/2+1)) < err.tf)
        ...
        current_error = err.stencil(soln.i,err.ptr(M/2+1)) ...
            - soln.calc_exact(soln.grid.x(soln.i),err.t(err.ptr(M/2+1)));
        ...
        if mod(err.count-1,out_interval)==0
    ...
    Primal.R = Primal.R(M/2+1:indP);
    Primal.R = cell2mat(Primal.R);
    % Primal.t = Primal.t(~isnan(Primal.t));
    Primal.t = Primal.t(M/2+1:indP);
    % Primal.E = Primal.E(~isnan(Primal.t),:,:);
    Primal.E = Primal.E(M/2+1:indP,:,:);
    for k = 1:N
        if ~isinf(p(k))
            Primal.Et(:,:,k) = (Primal.Et(:,:,k)/(soln.count-M)).^(1/p(k));
        end
    end
    ...
    for j = 1:soln.neq
        for k = 1:N
            ...
            Primal.Ef(1,j,k) = norm(Primal.E(:,j,k),p(k))/((indP-M/2)^(1/p(k)));
        end
    end
