...
%             this.u_old = soln_error.stencil;
        ...
        function [e_new,R,this] = step(this,soln,soln_error,bndry_cond)
            ...
            Ru = ss_residual(u(this.i),soln.grid.dx(this.i),soln.nu,soln.grid.imax);
            
            [u0,u13,soln_error] = soln_error.time_TE_est(t);
            [TE,~,~] = soln_error.space_TE_est(soln,u0);
            Tdudt = u13(soln.i);
            TE = TE + Tdudt;
            %             val = BDF2_ETE_res(u,e,em1,em2,Ru,TE,dx,dt,nu,N,BC1,BC2)
            F  = @(e) BDF2_ETE_res(u(this.i),e,this.em1,this.em2,Ru,TE,soln.grid.dx(this.i),soln.dt,soln.nu,soln.grid.imax,0,0);
%             val = BDF2_ETE_jac(u,e,dx,dt,nu,N)
            dF = @(e) BDF2_ETE_jac(u(this.i),e,soln.grid.dx(this.i),soln.dt,soln.nu,soln.grid.imax);
%             [xk,Rks] = newton_with_backtracking(x0,F,dF,gamma,res_tol,max_iter)
            [e_new,R] = newton_with_backtracking(e_new,F,dF,0.5,this.newton_tol,this.newton_max_iter);
            this.em2 = this.em1;
