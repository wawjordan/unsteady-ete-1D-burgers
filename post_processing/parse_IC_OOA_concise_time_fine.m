function [SE,PE,DEBUG] = parse_IC_OOA_concise_time_fine(filename)

load(filename,'OUT');

% get dimensions for output arrays
L = size(OUT.Error_Norms_E,2);        % (h) grid levels (space+time comb.)
M = size(OUT.Error_Norms_E(1,end).E,1); % (i) time levels (finest)
N = size(OUT.Error_Norms_E(1,end).E,2); % (j) iteration levels
O = 3;                                % (k) norms

t = OUT.Error_Norms_E(1,end).t(:);
...
stride = ones(L,1);
for h = L-1:-1:1
    stride(h) = (M-1)/(length(OUT.Error_Norms_E(1,h).t)-1);
end
...
for h = 1:L
    for i = 1:stride(h):M
        ...
        for j = 1:N
            for k = 1:O
                ETE_Ex(h,i,j,k) = OUT.Error_Norms_E(h).E(i2,j,k);
            end
        end
        for k = 1:O
            PRI_Ex(h,i,k) = OUT.Error_Norms_P(h).E(i2,k);
        end
    end
    for i = 1+stride(h):stride(h):M-stride(h)
        i2 = i+stride(h);
            for j = 1:N
                for k = 1:O
                    E1 = [ETE_Ex(h,i,j,k),ETE_Ex(h,i2,j,k)];
                    t1 = [t(i),t(i2)];
                    ETE_Ex(h,i+1:i2-1,j,k) = interp1(t1,E1,t(i+1:i2-1));
                end
            end
            for k = 1:O
                E1 = [PRI_Ex(h,i,k),PRI_Ex(h,i2,k)];
                t1 = [t(i),t(i2)];
                PRI_Ex(h,i+1:i2-1,k) = interp1(t1,E1,t(i+1:i2-1));
            end
    end
end
%         if i == 1
%             for j = 1:N
%                 for k = 1:O
%                     ETE_Ex(h,1:i2-1,j,k) = ETE_Ex(h,i2,j,k);
%                 end
%             end
%         else
% for h = 1:L
